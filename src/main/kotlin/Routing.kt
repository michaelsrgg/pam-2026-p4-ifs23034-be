...
fun Application.configureRouting() {
    ...
    val spiceService: SpiceService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Michael Saragih (11S23034).")
        }
        ...
        route("/spices") {
            get {
                spiceService.getAllSpices(call)
            }
            post {
                spiceService.createSpice(call)
            }
            get("/{id}") {
                spiceService.getSpiceById(call)
            }
            put("/{id}") {
                spiceService.updateSpice(call)
            }
            delete("/{id}") {
                spiceService.deleteSpice(call)
            }
            get("/{id}/image") {
                spiceService.getSpiceImage(call)
            }
        }
